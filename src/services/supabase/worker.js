...
async function insertWorker(name, age, ismale,phone) {
  const { data, error } = await supabase
    .from(WORKER)
    .insert([{ name, worker_age: age, ismale,phone}]);
...
async function getWorkers() {
  ...
  for (const w of workers) {
    const temp = new WorkerModel(w.worker_id, w.name, w.worker_age, w.ismale,w.phone);
    ret.push(temp);
  }
...
async function updateWorkerById(workerId, name, age, isMale,phone) {
  const { error } = await supabase
    .from(WORKER)
    .update({
      name,
